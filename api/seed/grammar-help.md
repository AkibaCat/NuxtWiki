...
使用 `[文本]{颜色*}` 语法为文字着色。颜色*（大括号内）支持 **十六进制**、**RGB**、**类名** 与 **Sass 变量** 四种写法：
...
- Sass 变量：`[示例]{$color}` → [示例]{$color} 套用页面样式表中定义的 `$color` 变量
- 主题色：`[示例]{$TC}` → [示例]{$TC} 内置主题色（`var(--ui-primary)`）
- 高亮内部仍可使用行内语法，例如 `[**加粗**]{$TC}` → [**加粗**]{$TC}

> 说明：十六进制可省略 `#` 前缀（`3c9c5c` 与 `#3c9c5c` 等价）；RGB 使用英文冒号分隔（红:绿:蓝），取值 0–255；`%` 后接类名（如 `%text` → `.text`），由本页样式表（“样式编辑器”）定义；`$` 后接 Sass 变量名，取页面样式表顶层变量；`$TC` 为内置特殊变量（站点主题色 `var(--ui-primary)`），不可被样式编辑器自定义。
