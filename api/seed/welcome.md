[![Nuxt v4](https://img.shields.io/badge/Made%20with-Nuxt%20v4-00DC82?logo=nuxt&labelColor=020420)](https://nuxt.com)

欢迎来到 **{{SITE_NAME}}** —— 一个轻量、快速、开箱即用的个人知识库 / 团队 Wiki。

> 本页面由安装向导自动生成，你可以随时编辑，把它改造成你的站点首页。

## 关于本站

{{SITE_NAME}} 采用「纯静态前端 + PHP API」架构：前端由 Nuxt 构建为纯静态页面，后端使用 PHP 直连数据库（PDO），无需 Node 运行时即可部署在 Kangle / Apache / Nginx 等任意虚拟主机上，秒开、稳定、易维护，几乎零管理成本。

## 核心特性

<details>
<summary>Markdown 编辑</summary>

- 标准语法：标题、加粗、斜体、列表、表格、引用、代码块、分割线
- 代码语法高亮与一键复制，让技术文档阅读更顺手
- 自定义文本着色：使用 `[{文本|颜色}]` 为文字上色，支持 6 位十六进制（如 `3c9c5c`）、RGB（如 `60:156:92`）以及主题色令牌 `$TC`（如 `[{NuxtWiki|$TC}]` 会以站点的主题色呈现）
- 支持 **HTML 白名单标签嵌套**：在 `div`、`details`、`table`、`span`、`mark` 等标签内部可以继续书写 Markdown，例如下面这个 `div` 容器：

<div class='wiki-note' style='border:1px solid #e5e7eb;border-radius:8px;padding:8px 16px;background:#f6f8fa'>

> 这是放在 `div` 容器里的一段引用，内部的 **Markdown 依然有效**。

- 项目一
- 项目二

</div>
</details>

<details>
<summary>沉浸式页面编辑器</summary>

- 沉浸式全屏编辑界面，去除干扰，专注写作
- 边写边看实时预览，所见即所得
- 支持多种编辑快捷操作，让排版更高效

</details>

<details>
<summary>版本管理</summary>

- 每次保存自动保留修订记录
- 随时查看历史、对比差异、一键回滚到任意版本

</details>

<details>
<summary>发现与检索</summary>

- 全文搜索、最近更改、随机页面
- 反向链接、贡献者榜，轻松发现相关内容

</details>

<details>
<summary>访问控制（ACL）</summary>

按页面分别设置阅读、编辑、历史、对比、回链、权限与贡献者的等级门槛，精细控制谁可以看、谁可以改。

</details>

<details>
<summary>订阅通知</summary>

订阅关注的页面，更新后第一时间收到通知，重要内容不再错过。

</details>

<details>
<summary>用户体系</summary>

- 通过注册码开放注册，控制成员准入
- 个人主页支持头像、简介、社交链接与活跃度图
- 支持冻结与封禁，灵活管理成员

</details>

<details>
<summary>主题定制</summary>

- 提供 24 套预设主题色，可自由切换
- 支持浅色 / 深色模式，自动适配使用环境
- 界面语言可选 简体中文 / 繁體中文 / English

</details>

<details>
<summary>管理后台</summary>

站点统计、站点设置、页面 / 用户 / 注册码管理，以及数据备份与导入，一站式掌控全站。

</details>

## 快速开始

1. 点击右上角「登录」，使用安装时设置的管理员账号登录
2. 点击「创建页面」或右上角「新建」，写下你的第一篇文档
3. 对照 [[语法帮助|GrammarHelp]] 学习完整的编辑语法
4. 在页面详情页点击「订阅」，持续关注页面更新

## 相关页面

- [[语法帮助|GrammarHelp]] —— 完整的 Markdown 与 HTML 嵌套语法教学

> 提示：本页内容可随时编辑，从这里开始你的 Wiki 之旅吧！